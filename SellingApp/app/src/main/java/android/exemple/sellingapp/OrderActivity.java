...
public class OrderActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String productID = "str" + intentOrder.getStringExtra("ID");
        initGlobalValues(productID);
        initialize();
    }
    ...
    private void initGlobalValues(String productID){
        ...
        */
        ...
        String[] data = getResources().getString(dataID).split(":");
        name = data[0];
        info = data[1];
        colors = new String[Integer.parseInt(data[2])];
        for(int i = 0; i < colors.length; i++){
            colors[i] = data[3+i];
        }
        price = Integer.parseInt(data[3+colors.length]);
